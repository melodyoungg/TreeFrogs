...
orientation_grav = [0,0,-1];            % [x,y,z] (put the 1 or -1 in the axis that aligns with up)
orientation_vel = [1,0,0];              % [x,y,z] (put 1 or -1 in the axis that aligns with vel vector)
substrate = 'Treadwall';
...
for k = 1:num_strides
    ...
    % getting current stride times and cropping data
    ...
    perrec = zeros(1,4);
    for j = 1:4
        % calculating kinetic energies [J/kg]
        ...
        EPraw = [Pxyz(:,1), 9.807.*(Pxyz(:,4)-Pxyz(1,4))];
        EPmin = abs(min(EPraw(:,2)));
        EP = EPraw;
        EP(:,2) = EP(:,2) + EPmin; 
    ...
    % calculating climbing efficiency of this stride
    ...
    power_matrix_actual = [PKxyz(:,2:end),PKtot(:,2),PP(:,2),Ptot(:,2)];
    power_mean = mean(power_matrix_actual,1);
    
    % calculating sum of positive energy increments [Ex,Ey,Ez,EK,EP,Etot]
    sum_power_matrix = [EKxyz(:,2:4),EKtot(:,2),EP(:,2),Etot(:,2)];
    sum_power = zeros(size(sum_power_matrix));
    for j = 1:size(sum_power_matrix,2)
        for i = 2:size(sum_power_matrix,1)
            sum_power(i,j) = sum_power_matrix(i,j) - sum_power_matrix(i-1,j);
        end
    end
    sum_power(sum_power < 0) = 0;
    sum_power = sum(sum_power);
    ...
    if abs(orientation_grav(3)) == 1         % if horizontal
        spaciotemp_labels = {'Filename','Stride No.','Orientation','Substrate',...
            ...
            'avgPx [W]','avgPy [W]','avgPz [W]', 'avgPK [W]', 'avgPP [W]', 'avgPtot [W]'};
    else                                % if vertical
        spaciotemp_labels = {'Filename','Stride No.','Orientation','Substrate',...
    ...
    if abs(orientation_grav(3)) == 1 % if horizontal, calc phase
        % displays graph and gets user input
        %fig = figure(1);
        plot(EKtot(:,1),EKtot(:,2),EP(:,1),EP(:,2))
        legend('Kinetic','Potential')
        title('click the peak of KE, then of PE')
        [peak_trough,~] = ginput(2);
%         close(fig)
        
        % calculates phase
        peak = peak_trough(1);
        trough = peak_trough(2);
        phase = {abs(180 - abs(peak-trough)/(EP(end,1)-EP(1,1))*360)};
    else
        phase = {'N/A'};
    end
    ...
    power_mean = power_mean * subject_mass;
    ...
    % exporting data, goes into proper COM Data folder
    cd('Force8')
    cd('COM Data')
    ...
    % filenames
    COM_filename = strcat(event_name,'_COM.xlsx');
    scaled_filename = strcat(event_name,'_Scaled.xlsx');
    spaciotemp_filename = strcat(event_name,'_Spaciotemp.xlsx');
...
for j = 2:length(COMlabels)
    ...
    disp(strcat('Aproximately_',num2str(percent_done),'% Complete'));
end
